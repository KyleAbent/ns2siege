...
local function UpdateItemsGUIScale(self)
    ...
    kBorderMaskCircleRadius = GUIScale(145)
    ...
    kSmokeyBackgroundSize = GUIScale(Vector(256, 130, 0))
...
local function UpdateUnitStatusBlip( self, blipData, updateBlip, localPlayerIsCommander, baseResearchRot, showHints, playerTeamType )
    ...
    if updateBlip.BorderMask then
        if teamType == kMarineTeamType and showBacking then            
            ...
            updateBlip.Border:SetIsVisible(true)
        else
            updateBlip.Border:SetIsVisible(false)
        end
    end
...
local function FindUnitsToDisplayStatusFor(player)
    ...
    if crosshairTarget
        ...
        local distance = (player:GetEyePos() - crosshairTarget:GetOrigin()):GetLength()
        if distance < GUIUnitStatus.kMaxUnitStatusDistance then
    ...
    if isOverheadSpec then
        ...
        for _, target in ipairs(GetEntitiesWithMixinWithinRange("UnitStatus", player:GetOrigin(), GUIUnitStatus.kMaxUnitStatusDistance )) do
            if (target ~= player and HasMixin(target, "UnitStatus") and GetIsInFov(player, target, fov)) then
...
// do updates about 12 times per second
local kUnitStatusUpdateInterval = 0.08
