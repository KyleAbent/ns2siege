...
local kDefaultFontName = Fonts.kArial_15 
...
local function UpdateBackGroundSize(self)
    ...
    local stringWidth = GUIScale(self.text:GetTextWidth(self.storedString))
    ...
    bgSize.y = GUIScale(self.text:GetTextHeight(self.storedString)) + self.textPadding[TOP] + self.textPadding[BOTTOM]
...
function Font:SetIsScaling(isScaling)
    ...
    self.text:SetTextClipped(self.isClipped, self.textWidth, self.textHeight)
