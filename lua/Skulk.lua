...
//    size = 2
//    end
    return Skulk.kViewOffsetHeight //* size
end
function Skulk:GetExtentsOverride()
 if self.modelsize < 1 then
    return Vector(Skulk.kXExtents * self.modelsize, Skulk.kYExtents * self.modelsize, Skulk.kZExtents * self.modelsize)
  else
   return Vector(Skulk.kXExtents, Skulk.kYExtents, Skulk.kZExtents)
   end
end
...
/*
function Skulk:OnKill(self, damage, attacker, doer, point, direction)
           if GetHasTech(self, kTechId.SkulkRupture) then
...
function Skulk:ModifyJump(input, velocity, jumpVelocity)
    ...
    if self:GetCanWallJump() then
        ...
        local celerityMod = (GetHasCelerityUpgrade(self) and ConditionalValue(self.RTDCelerity == true, 3, GetSpurLevel(self:GetTeamNumber())) or 0) * 0.4
...
function Skulk:GetAirFriction()
    return 0.055 - (GetHasCelerityUpgrade(self) and ConditionalValue(self.RTDCelerity == true, 3, GetSpurLevel(self:GetTeamNumber())) or 0) * 0.009
end 
