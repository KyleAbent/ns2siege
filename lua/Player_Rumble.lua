...
function Player:GetRandomNearbyLight2(range)
  
    PROFILE("Player:GetRandomNearbyLight2")
    ...
    if self.shakingLightList then
        ...
        for i = 0, 10 do
            ...
            if currentLight and not dirtyLightCache[currentLight] == false then
                       
                if (currentLight.originalCoords.origin - self:GetOrigin()):GetLength() < range then
                
                    if dirtyLightCache[currentLight] == None then
                        ...
                        dirtyLightCache[currentLight] = trace.fraction ~= 1 and trace.entity == nil
                    
                    end
                  
                    if dirtyLightCache[currentLight] then
...
// Pick random light within range
function Player:GetRandomNearbyLight1(range)

    PROFILE("Player:GetRandomNearbyLight1")
...
useNearbyLight1 = true

Event.Hook("Console_unl", function() 
    useNearbyLight = not useNearbyLight
    Log("unl = %s", useNearbyLight)
end)

function Player:GetRandomNearbyLight(range)
    if useNearbyLight1 then
        return self:GetRandomNearbyLight1(range)
    end
    return self:GetRandomNearbyLight2(range)
end
...
function Player:UpdateDirtFalling(deltaTime)
    ...
    PROFILE("Player:UpdateDirtFalling")
