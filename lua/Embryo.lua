...
function Embryo:SetGestationData(techIds, previousTechId, healthScalar, armorScalar)
    ...
    /*
    if GetHasHyperMutationUpgrade(self) or table.contains(self.evolvingUpgrades, kTechId.HyperMutation) then
        local prevGestationTime = self.gestationTime
        self.gestationTime = math.max(0, self.gestationTime - self.storedHyperMutationTime)
        self.storedHyperMutationTime = math.max(self.storedHyperMutationTime, prevGestationTime)
    else
        self.storedHyperMutationTime = 0
    end
    */
