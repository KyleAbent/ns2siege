...
function GUIGorgeBuildMenu:Reset()
    ...
    for i = 1, 5 do 
     local structureAbility = DropStructureAbility.kSupportedStructures[i]
        // TODO: pass keybind from options instead of index
        table.insert( self.buttons, self:CreateButton(structureAbility.GetDropStructureId(), self.scale, self.background, GorgeBuild_GetKeybindForIndex(i), i - 1) )
