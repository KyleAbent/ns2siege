...
local function OnCommandVortex(client)

    local player = client:GetControllingPlayer()
    
    if Shared.GetCheatsEnabled() and player and player.SetVortexDuration then
        player:SetVortexDuration(4)
    end
...
local function OnCommandGoThere(client)
    ...
    if Shared.GetCheatsEnabled() then
        ...
        selected,selectedRange = FindNearestAIUnit(player)
...
Event.Hook("Console_vortex", OnCommandVortex)
