...
if Client then
    ...
    function Order:GetMapBlipType()
        return self.mapBlipType or kMinimapBlipType.MoveOrder
    end
    
    function Order:GetMapBlipColor(minimap, item)
        return item.blipColor
    end
    
    function Order:GetMapBlipTeam(playerTeam)
        return kMinimapBlipTeam.Friendly
    end
    
    function Order:GetMapBlipOriginOverride()
        return self:GetLocation()
    end
    
    function Order:UpdateMinimapActivity(minimap, item)
        if self.ownerId ~= Client.GetLocalPlayer():GetId() then
            -- other peoples order; we probably won't see them anyhow because they are not relevant to us
            return nil
        end
        local blipType = kMinimapBlipType.MoveOrder
        local orderType = self:GetType()
...
// This class does not seem to be used; TODO: REMOVE DEAD CODE
