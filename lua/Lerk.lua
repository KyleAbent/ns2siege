...
if Server then
    function Lerk:OnDamageDone(doer, target)
    
        
            if doer:isa("LerkBite") and target:isa("Player") and GetHasTech(self, kTechId.Umbra) then
               self:SetHasUmbra(true,kUmbraRetainTime)
            end
...
function Lerk:GetExtentsOverride()
 //if self.modelsize < 1 then
  //  return Vector(Lerk.XZExtents * self.modelsize, Lerk.YExtents * self.modelsize, Lerk.XZExtents * self.modelsize)
  //else
      return Vector(Lerk.XZExtents, Lerk.YExtents, Lerk.XZExtents)
  // end
end
function Lerk:GetAirFriction()
    return 0.1 - (GetHasCelerityUpgrade(self) and ConditionalValue(self.RTDCelerity == true, 3, GetSpurLevel(self:GetTeamNumber())) or 0) * 0.02
end
...
function Lerk:OnWorldCollision(normal)
    ...
    self.wallGripAllowed = normal.y < 0.5 and not self:GetCrouching() and not self.occupied
...
local function UpdateFlap(self, input, velocity)
    ...
    if flapPressed ~= self.flapPressed then
        ...
        local fapenergy = kLerkFlapEnergyCost
        if self.lerkusinggorgeID ~= Entity.invalidI then fapenergy = kLerkFlapEnergyCost * kLerkLiftFlapEnergyMultiplier end
        if flapPressed and self:GetEnergy() > fapenergy and not self.gliding then
    ...
    */
                local maxSpeedTable = { maxSpeed = kMaxSpeed } //* size }
                ...
                elseif input.move.z == 1 and input.move.x == 0 then
                    flapForce = 3 + flapForce + (GetHasCelerityUpgrade(self) and ConditionalValue(self.RTDCelerity == true, 3, GetSpurLevel(self:GetTeamNumber())) or 0) * 0.3               
...
local function UpdateAirStrafe(self, input, velocity, deltaTime)
    ...
    */
        local maxSpeed = math.max(kAirStrafeMaxSpeed, velocity:GetLengthXZ()) //* size, velocity:GetLengthXZ())        
...
function Lerk:OnKill()
   ...
   local gorge = Shared.GetEntity( self.lerkcarryingGorgeId ) 
      if gorge then
       gorge:SetOrigin(self:GetOrigin())
