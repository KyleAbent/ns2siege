...
function ModularJetpack_ConvertNetMessageToConfig(message)
    return {
        [kJetpackModuleSlots.PowerSupply] = message.powerModuleType    or kExoModuleTypes.None,
        [kJetpackModuleSlots.LeftArm    ] = message.leftArmModuleType  or kExoModuleTypes.None,
        [kJetpackModuleSlots.RightArm   ] = message.rightArmModuleType or kExoModuleTypes.None,
        [kJetpackModuleSlots.Armor      ] = message.armorModuleType    or kExoModuleTypes.None,
        [kJetpackModuleSlots.Utility    ] = message.utilityModuleType  or kExoModuleTypes.None,
    }
end

function ModularJetpack_ConvertConfigToNetMessage(config)
    return {
        powerModuleType    = config[kJetpackModuleSlots.PowerSupply] or kJetpackModuleSlots.None,
        leftArmModuleType  = config[kJetpackModuleSlots.LeftArm    ] or kJetpackModuleSlots.None,
        rightArmModuleType = config[kJetpackModuleSlots.RightArm   ] or kJetpackModuleSlots.None,
        armorModuleType    = config[kJetpackModuleSlots.Armor      ] or kJetpackModuleSlots.None,
        utilityModuleType  = config[kJetpackModuleSlots.Utility    ] or kJetpackModuleSlots.None,
    }
end

function ModularJetpack_GetIsConfigValid(config)
    local resourceCost = 0
    local powerCost = 0
    local powerSupply = nil -- We don't know yet
    local leftArmType = nil
    local rightArmType = nil
    for slotType, slotTypeData in pairs(kJetpackModuleSlots) do
        local moduleType = config[slotType]
        if moduleType == nil or moduleType == kJetpackModuleSlots.None then
            if slotTypeData.required then
                -- The config MUST give a module type for this slot type
                return false, "missing required slot" -- not a valid config
            else
            ...
            -- The config has module type for this slot type
            local moduleTypeData = kJetpackModuleSlots[moduleType]
            if moduleTypeData == nil or moduleTypeData.category ~= slotTypeData.category then
                -- They have provided the wrong category of module type for this slot type
                -- For example, an armor module in a weapon slot
                return false, "wrong slot type" -- not a valid config
            ...
            if slotType == kJetpackModuleSlots.LeftArm then
                leftArmType = moduleTypeData.armType
            elseif slotType == kJetpackModuleSlots.RightArm then
                rightArmType = moduleTypeData.armType
            end
        end
    end
...
function ModularJetpack_GetConfigWeight(config)
    local weight = 0
    for slotType, slotTypeData in pairs(kExoModuleSlotsData) do
        local moduleType = config[slotType]
        if moduleType and moduleType ~= kExoModuleTypes.None then
