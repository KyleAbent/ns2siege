...
function Fade:GetAirFriction()
    return (self:GetIsBlinking() or self:GetRecentlyShadowStepped()) and 0 or (0.17  - (GetHasCelerityUpgrade(self) and ConditionalValue(self.RTDCelerity == true, 3, GetSpurLevel(self:GetTeamNumber())) or 0) * 0.01)
end 
...
function Fade:GetRecentlyBlinkAltered()
return self.recentlyblinked
end
...
function Fade:TriggerShadowStep(direction)
    ...
    */
    ...
    if not self:GetIsBlinking() and not self:GetHasShadowStepCooldown() and self:GetEnergy() > kFadeShadowStepCost then
        ...
        local celerityAddSpeed = (GetHasCelerityUpgrade(self) and ConditionalValue(self.RTDCelerity == true, 3, GetSpurLevel(self:GetTeamNumber())) or 0) * 0.7
