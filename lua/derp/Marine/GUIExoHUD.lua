...
function GUIExoHUD:Initialize()
    ...
    self.crosshair:SetPosition(Vector(-GUIScale(32), -GUIScale(32), 0))
...
local function UpdateTargets(self)
    ...
    for t = 1, #trackEntities do
        ...
        if inFront and trackEntity:GetIsAlive() and ( trackEntity:isa("Skulk") and not trackEntity:GetIsCloaked() ) or (trackEntity:isa("Fade") and  GetHasSilenceUpgrade(trackEntity) and trackEntity:GetIsBlinking() or trackEntity:GetRecentlyBlinkAltered() )then
