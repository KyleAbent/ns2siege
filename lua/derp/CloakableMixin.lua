...
function CloakableMixin:GetIsEligableToBeInvisible()
if self:isa("Fade") then
return self:GetIsBlinking() or self:GetRecentlyBlinkAltered() 
else
return false
...
local function UpdateDesiredCloakFraction(self, deltaTime)
    ...
    if not ( self:GetIsEligableToBeInvisible() ) then 
...
elseif Client then
    ...
    function CloakableMixin:_UpdatePlayerModelRender(model)
        ...
        local useMaterial = (self.cloakingDesired or self:GetCloakFraction() ~= 0  or self:GetIsEligableToBeInvisible() ) and not hideFromEnemy
...
function CloakableMixin:SecondaryAttack()
    ...
    if weapon and weapon:GetHasSecondary(self) and not ( self:GetIsEligableToBeInvisible() ) then
