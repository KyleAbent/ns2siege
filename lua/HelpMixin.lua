...
local kCheckMarkFileName = PrecacheAsset("ui/checkmark.dds")
            ...
            if self.activeHelpWidget == nil then
                ...
                if self:GetTeamNumber() ~= kNeutralTeamType then
                    ...
                    if currentAmount < limit then
                        ...
                        // might as well run all help scripts at full rate; only run at most one and only if hints are on
                        // ... and there are loads of them...
                        self.activeHelpWidget.updateInterval = 0
                    end
