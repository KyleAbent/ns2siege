 ...
 */
function AlienBuy_GetAlienCost(alienType, hasHyperMutation)

    assert(hasHyperMutation ~= nil)
...
function GetAlienTierThreeFor(techId)
    ...
    if not gTierThreeTech then
        ...
        gTierThreeTech[kTechId.Fade] = kTechId.Stab and kTechId.AcidRocket
...
function AlienBuy_GetHyperMutationCostReduction(alienType)
    ...
    local player = Client.GetLocalPlayer()
    if player and GetHasHyperMutationUpgrade(player) then        
        costReduction = player.storedHyperMutationCost
    end
