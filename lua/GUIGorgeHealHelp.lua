...
function GUIGorgeHealHelp:OnResolutionChanged(oldX, oldY, newX, newY)
    self:Uninitialize()
    self:Initialize()
end
...
local function GetHealingRequired(player)
    ...
    for s = 1, #status do
        ...
        local entity = Shared.GetEntity(status[s].UnitId)
        local entityOrigin = entity and entity:GetOrigin() or nil
        if (unitStatus == kUnitStatus.Damaged or unitStatus == kUnitStatus.Unbuilt) and entityOrigin and 
           (entityOrigin - player:GetEyePos()):GetLengthSquared() <= (kHealsprayRadius * kHealsprayRadius) then
            return true
        end
...
function GUIGorgeHealHelp:Update(dt)
    ...
    PROFILE("GUIGorgeHealHelp:Update")
