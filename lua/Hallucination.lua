...
Script.Load("lua/CommAbilities/Alien/HallucinationExplosion.lua")
...
local networkVars =
{
    ...
    creationTime = "time",
    controlledhallucination = "boolean",
}
...
function Hallucination:OnInitialized()
    ...
    self.controlledhallucination = false
end
...
function Hallucination:PerformActivation(techId, position, normal, commander)
    ...
    elseif techId == kTechId.HallucinationExplosion then
        self:TriggerAwesomeExplosion()
    end
    return false, true
...
function Hallucination:TriggerAwesomeExplosion()
     self:TriggerEffects("hallucinationexplosion", {effecthostcoords = Coords.GetTranslation(self:GetOrigin())})
    local hitEntities = GetEntitiesWithMixinWithinRange("Live", self:GetOrigin(), kXenocideRange)
    RadiusDamage(hitEntities, self:GetOrigin(), kXenocideRange, kXenocideDamage, self)
    DestroyEntity(self)
   // self:TriggerEffects("crag_trigger_umbra") < maybe some other cool effect?
    return true
end
function Hallucination:GetTechButtons(techId)

    local techbuttons =  { kTechId.DestroyHallucination, kTechId.None, kTechId.None, kTechId.None,
                           kTechId.None,                  kTechId.None,  kTechId.None, kTechId.None }
    
    if self.controlledhallucination then
    techbuttons[1] = kTechId.HallucinationExplosion
    end
