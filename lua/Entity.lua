...
if Predict then
    ...
    function Entity:AddTimedCallback(callback, interval, early)
        -- default to early 
        early = early ~= nil and early or true 
...
function GetEntitiesForTeamWithinRange(className, teamNumber, origin, range)
    ...
    if not className == "PhaseGate" then
...
function GetEntitiesForTeamWithinXZRangeMixin(mixinType, teamNumber, origin, range)
    ...
    return GetEntitiesWithFilter(Shared.GetEntitiesWithTag(mixinType), inRangeXZFilterFunction)
...
function GetIsInFov(seeingEntity, target, fov)
    
    local targetOrigin = HasMixin(targetEntity, "Target") and target:GetEngagementPoint() or target:GetOrigin()
