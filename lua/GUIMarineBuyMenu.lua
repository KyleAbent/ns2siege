...
function GUIMarineBuyMenu:_UpdateContent(deltaTime)
   ...
   /*
    if self.hoverItem == kTechId.Jetpack or (self.hoverItem == nil and self.hoveringJetpack) then
        self.hoveringJetpack = true
        self.portrait:SetIsVisible(true)
		self.portrait:SetTexturePixelCoordinates(GetBigIconPixelCoords(kTechId.Jetpack))
        self.itemName:SetIsVisible(false)
        self.itemDescription:SetIsVisible(false)
        self.ModularJetpackConfigActive = true
        for elementI, element in ipairs(self.ModularJetpackGraphicItemsToDestroyList) do
            element:SetIsVisible(true)
        end
        
        return
    end
    if self.ModularJetpackGraphicItemsToDestroyList then
        self.hoveringJetpack = false
        self.ModularJetpackConfigActive = false
        for elementI, element in ipairs(self.ModularJetpackGraphicItemsToDestroyList) do
            element:SetIsVisible(false)
        end
    end
    ...
    */
    ...
    ///modular
      ...
      ///  return false, false
        ...
        if self.hoveringJetpack then
            if GetIsMouseOver(self, self.ModularJetpackBuyButton) and MarineBuy_IsResearched(kTechId.Exosuit) then
                Client.SendNetworkMessage("JetpackModularBuy", ModularJetpack_ConvertConfigToNetMessage(self.JetpackConfig))
                MarineBuy_OnClose()
                return true, true
            end
            for buttonI, buttonData in ipairs(self.modularJetpackModuleButtonList) do
                if GetIsMouseOver(self, buttonData.buttonGraphic) then
                    if buttonData.state == "enabled" then
                        self.JetpackConfig[buttonData.slotType] = buttonData.moduleType
                        if buttonData.forceToDefaultConfig then
                            self.JetpackConfig[kJetpackModuleSlots.RightArm] = kJetpackModuleTypes.Minigun
                            self.JetpackConfig[kJetpackModuleSlots.LeftArm ] = kJetpackModuleTypes.Claw
                            self.JetpackConfig[kJetpackModuleSlots.Armor   ] = kJetpackModuleTypes.None
                            self.JetpackConfig[kJetpackModuleSlots.Utility ] = kJetpackModuleTypes.None
                        end
                        if buttonData.forceLeftToClaw then
                            self.JetpackConfig[kJetpackModuleSlots.LeftArm] = kJetpackModuleTypes.Claw
                        end
...
/*
function  GUIMarineBuyMenu:_InitializeJetpackModularButtons()
    self.activeJetpackConfig = nil
    local player = Client.GetLocalPlayer()
    if player and player:isa("JetpackMarine") then
        self.activeJetpackConfig = ModularJetpack_ConvertNetMessageToConfig(player)
        local isValid, badReason, resourceCost, powerSupply = ModularJetpack_GetIsConfigValid(self.activeJetpackConfig)
        self.activeJetpackConfigResCost = resourceCost
        self.activeJetpackConfigPowerSupply = powerSupply
        self.JetpackConfig = self.activeJetpackConfig
    else
        self.activeJetpackConfig = {}
        self.activeJetpackConfigResCost = 0
        self.activeJetpackConfigPowerSupply = 0
        self.JetpackConfig = {
            [kJetpackModuleSlots.PowerSupply] = kJetpackModuleTypes.Power1,
            [kJetpackModuleSlots.RightArm   ] = kJetpackModuleTypes.Minigun,
            [kJetpackModuleSlots.LeftArm    ] = kJetpackModuleTypes.Claw,
            [kJetpackModuleSlots.Armor      ] = kJetpackModuleTypes.None,
            [kJetpackModuleSlots.Utility    ] = kJetpackModuleTypes.None,
        }
    end
    
    self.ModularJetpackConfigActive = false
    self.ModularJetpackGraphicItemsToDestroyList = {} -- WWHHYY UWE, WWWHHHHYYYYYY?!?!¿!?
    self.modularJetpackModuleButtonList = {}
    
    self.ModularJetpackBuyButton = GUIManager:CreateGraphicItem()
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, self.ModularJetpackBuyButton)
    self.ModularJetpackBuyButton:SetAnchor(GUIItem.Left, GUIItem.Top)
    self.ModularJetpackBuyButton:SetSize(Vector(GUIMarineBuyMenu.kButtonWidth*1.5, GUIMarineBuyMenu.kButtonHeight, 0))
    self.ModularJetpackBuyButton:SetPosition(Vector(
    ...
    self.ModularJetpackBuyButton:SetTexture(GUIMarineBuyMenu.kButtonTexture)
    self.ModularJetpackBuyButton:SetLayer(kGUILayerPlayerHUDForeground4)
    self.content:AddChild(self.ModularJetpackBuyButton)
    
    self.ModularJetpackBuyButtonText = GUIManager:CreateTextItem()
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, self.ModularJetpackBuyButtonText)
    self.ModularJetpackBuyButtonText:SetAnchor(GUIItem.Middle, GUIItem.Center)
    self.ModularJetpackBuyButtonText:SetPosition(Vector(-GUIMarineBuyMenu.kPadding*5, 0, 0))
    self.ModularJetpackBuyButtonText:SetFontName(GUIMarineBuyMenu.kFont)
    self.ModularJetpackBuyButtonText:SetTextAlignmentX(GUIItem.Align_Center)
    self.ModularJetpackBuyButtonText:SetTextAlignmentY(GUIItem.Align_Center)
    self.ModularJetpackBuyButtonText:SetText("UPGRADE")
    self.ModularJetpackBuyButtonText:SetFontIsBold(true)
    self.ModularJetpackBuyButtonText:SetColor(GUIMarineBuyMenu.kCloseButtonColor)
    self.ModularJetpackBuyButton:AddChild(self.ModularJetpackBuyButtonText)
    
    self.ModularJetpackCostText = GUIManager:CreateTextItem()
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, self.ModularJetpackCostText)
    self.ModularJetpackCostText:SetAnchor(GUIItem.Right, GUIItem.Center)
    self.ModularJetpackCostText:SetPosition(Vector(-GUIMarineBuyMenu.kPadding*7, 0, 0))
    self.ModularJetpackCostText:SetFontName(GUIMarineBuyMenu.kFont)
    self.ModularJetpackCostText:SetTextAlignmentX(GUIItem.Align_Min)
    self.ModularJetpackCostText:SetTextAlignmentY(GUIItem.Align_Center)
    self.ModularJetpackCostText:SetText("69")
    self.ModularJetpackCostText:SetFontIsBold(true)
    self.ModularJetpackCostText:SetColor(GUIMarineBuyMenu.kTextColor)
    self.ModularJetpackBuyButton:AddChild(self.ModularJetpackCostText)
    
    self.ModularJetpackCostIcon = GUIManager:CreateGraphicItem()
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, self.ModularJetpackCostIcon)
    self.ModularJetpackCostIcon:SetSize(Vector(GUIMarineBuyMenu.kResourceIconWidth * 0.8, GUIMarineBuyMenu.kResourceIconHeight * 0.8, 0))
    self.ModularJetpackCostIcon:SetAnchor(GUIItem.Right, GUIItem.Center)
    self.ModularJetpackCostIcon:SetPosition(Vector(-GUIMarineBuyMenu.kPadding*11, -GUIMarineBuyMenu.kResourceIconHeight*0.4, 0))
    self.ModularJetpackCostIcon:SetTexture(GUIMarineBuyMenu.kResourceIconTexture)
    self.ModularJetpackCostIcon:SetColor(GUIMarineBuyMenu.kTextColor)
    self.ModularJetpackBuyButton:AddChild(self.ModularJetpackCostIcon)
    
    for slotType, slotGUIDetails in pairs(GUIMarineBuyMenu.kJetpackSlotData) do
        local panelBackground = GUIManager:CreateGraphicItem()
        table.insert(self.ModularJetpackGraphicItemsToDestroyList, panelBackground)
        ...
        if slotType == "StatusPanel" then
            ...
            table.insert(self.ModularJetpackGraphicItemsToDestroyList, weightLabel)
            ...
            table.insert(self.ModularJetpackGraphicItemsToDestroyList, note)
            ...
            table.insert(self.ModularJetpackGraphicItemsToDestroyList, weight)
            ...
            self.ModularJetpackWeightLabel = weightLabel
        else
            local slotTypeData = kJetpackModuleSlotsData[slotType]
            
            local panelTitle = GetGUIManager():CreateTextItem()
            table.insert(self.ModularJetpackGraphicItemsToDestroyList, panelTitle)
            ...
            for moduleType, moduleTypeName in ipairs(kJetpackModuleTypes) do
                ...
                if moduleType == kJetpackModuleTypes.None and not slotTypeData.required then
                    isSameType = true
                    moduleTypeData = {}
                end
                if isSameType then
...
function GUIMarineBuyMenu:MakePowerModuleButton(moduleType, moduleTypeData, offsetX, offsetY)
    local moduleTypeGUIDetails = GUIMarineBuyMenu.kJetpackModuleData[moduleType]
    local powerSupply = moduleTypeData.powerSupply
    local disabled = false
    if powerSupply < self.activeJetpackConfigPowerSupply then
        disabled = true
    end
    local buttonGraphic = GUIManager:CreateGraphicItem()
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, buttonGraphic)
    ...
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, powerSupplyLabel)
    ...
    powerSupplyLabel:SetText(disabled and "---" or "+"..tostring(powerSupply-self.activeJetpackConfigPowerSupply))--(Locale.ResolveString("BUY"))
    buttonGraphic:AddChild(powerSupplyLabel)
    
    local powerIcon = GUIManager:CreateGraphicItem()
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, powerIcon)
    ...
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, resCostLabel)
    ...
    resCostLabel:SetText(disabled and "---" or tostring(moduleTypeData.resourceCost-self.activeJetpackConfigResCost))--(Locale.ResolveString("BUY"))
    buttonGraphic:AddChild(resCostLabel)
    
    local resIcon = GUIManager:CreateGraphicItem()
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, resIcon)
    ...
    table.insert(self.modularJetpackModuleButtonList, { -- we need to keep this list so it can change their colour
        powerSupply = moduleTypeData.powerSupply,
        slotType = kJetpackModuleSlots.PowerSupply,
...
function GUIMarineBuyMenu:MakeWeaponModuleButton(moduleType, moduleTypeData, offsetX, offsetY, slotType)
    ...
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, weaponLabel)
    ...
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, weaponImage)
    ...
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, powerCostLabel)
   ...
   // powerIcon:SetTexturePixelCoordinates(iconX*80, iconY*80, iconX*80+80, iconY*80+80)
    ...
    table.insert(self.modularJetpackModuleButtonList, {
        slotType = slotType,
        moduleType = moduleType,
        buttonGraphic = buttonGraphic,
...
function GUIMarineBuyMenu:MakeUtilityModuleButton(moduleType, moduleTypeData, offsetX, offsetY, slotType)
    ...
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, utilityLabel)
    ...
    table.insert(self.ModularJetpackGraphicItemsToDestroyList, utilityImage)
   ...
   // powerIcon:SetTexturePixelCoordinates(iconX*80, iconY*80, iconX*80+80, iconY*80+80)
    ...
    table.insert(self.modularJetpackModuleButtonList, {
        slotType = kJetpackModuleSlots.Utility,
        moduleType = moduleType,
        buttonGraphic = buttonGraphic,
...
/*
function GUIMarineBuyMenu:_UpdateJetpackModularButtons(deltaTime)
    if self.hoveringJetpack then
        self:_RefreshJetpackModularButtons()
        if not MarineBuy_IsResearched(kTechId.Jetpack) or PlayerUI_GetPlayerResources() < self.JetpackConfigResourceCost-self.activeJetpackConfigResCost then
            self.ModularJetpackBuyButton:SetColor(Color(1, 0, 0, 1))
            
            self.ModularJetpackBuyButtonText:SetColor(Color(0.5, 0.5, 0.5, 1))
            self.ModularJetpackCostText:SetColor(GUIMarineBuyMenu.kCannotBuyColor)
            self.ModularJetpackCostIcon:SetColor(GUIMarineBuyMenu.kCannotBuyColor)
        else
            if GetIsMouseOver(self, self.ModularJetpackBuyButton) then
                self.ModularJetpackBuyButton:SetColor(Color(1, 1, 1, 1))
            else
                self.ModularJetpackBuyButton:SetColor(Color(0.5, 0.5, 0.5, 1))
            end
...
function GUIMarineBuyMenu:_RefreshJetpackModularButtons()
    local isValid, badReason, resourceCost, powerSupply, powerCost, texturePath = ModularJetpack_GetIsConfigValid(self.JetpackConfig)
    resourceCost = resourceCost or 0
    self.JetpackConfigResourceCost = resourceCost
    if not self.activeJetpackConfigResCost then self.activeJetpackConfigResCost = 0 end
    self.ModularJetpackCostText:SetText(tostring(resourceCost-self.activeJetpackConfigResCost))
    //self.ModularJetpackPowerUsageLabel:SetText(tostring(powerCost).." of "..tostring(powerSupply))
    self.JetpackConfigWeight = ModularJetpack_GetConfigWeight(self.JetpackConfig)
    local weightLabel, weightCol = "?!?", Color(1, 0.7, 0.7, 1)
    for weightClassI, weightClass in ipairs(GUIMarineBuyMenu.kWeightLabelData) do
        if self.JetpackConfigWeight >= weightClass.min then
            weightLabel, weightCol = weightClass.label, weightClass.col
        end
    end
    self.ModularJetpackWeightLabel:SetText(tostring(weightLabel))
    self.ModularJetpackWeightLabel:SetColor(weightCol)
    
    for buttonI, buttonData in ipairs(self.modularJetpackModuleButtonList) do
        local current = self.JetpackConfig[buttonData.slotType]
        local col = nil
        local canAfford = true
        if current == buttonData.moduleType then
            if PlayerUI_GetPlayerResources() < self.JetpackConfigResourceCost-self.activeJetpackConfigResCost then
            ...
            self.JetpackConfigJetpackConfig[buttonData.slotType] = buttonData.moduleType
            local isValid, badReason, resourceCost, powerSupply, powerCost, texturePath = ModularJetpack_GetIsConfigValid(self.JetpackConfig)
            if buttonData.slotType == kJetpackModuleSlots.PowerSupply then
                if buttonData.powerSupply < self.activeJetpackConfigPowerSupply then
                    isValid = false
                    badReason = "no refunds!"
                elseif isValid then
                    resourceCost = resourceCost-self.activeJetpackConfigResCost
            ...
            if buttonData.slotType == kJetpackModuleSlots.RightArm and badReason == "bad model left" then
                isValid = true
            ...
            self.JetpackConfig[buttonData.slotType] = current
        end
        buttonData.col = col
