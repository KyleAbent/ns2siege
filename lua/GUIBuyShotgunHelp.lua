...
function GUIBuyShotgunHelp:OnResolutionChanged(oldX, oldY, newX, newY)
    self:Uninitialize()
    self:Initialize()
end
...
function GUIBuyShotgunHelp:Initialize()
    ...
    self.shotgunImage:SetPosition(Vector(-kIconWidth / 2, -kIconHeight + GUIScale(kHelpBackgroundYOffset), 0))
    self.shotgunImage:SetSize(GUIScale(kIconSize))
    ...
    self.arrow:SetSize(GUIScale(kArrowSize))
    self.arrow:SetPosition(GUIScale(Vector(-kArrowSize.x / 2, -kArrowSize.y, 0)))
...
function GUIBuyShotgunHelp:Update(dt)
    ...
    PROFILE("GUIBuyShotgunHelp:Update")
        ...
        if player and shotgunResearched and enoughResources then
            ...
            if armory then
                ...
                if not nearbyUnitsUnderAttack then
                    ...
                    local goalPos = Vector(Client.GetScreenWidth() / 2 - GUIScale(kIconSize.x / 2), Client.GetScreenHeight() + GUIScale(kHelpBackgroundYOffset - kIconSize.y), 0)
                    ...
                    if inFront then
                        goalPos = displayPoint - GUIScale(kIconSize) / 2
                    else
                    ...
                    if player:GetBuyMenuIsDisplaying() then
                        ...
                        HelpWidgetIncreaseUse(self, "GUIBuyShotgunHelp")
