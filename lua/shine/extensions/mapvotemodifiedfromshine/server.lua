...
function Plugin:ForcePlayersIntoReadyRoom()//MarineTeam
	local Gamerules = GetGamerules()

	local function MoveToMarines( Player )
		Gamerules:JoinTeam( Player, 1, nil, true ) // 1
	end

	Gamerules.team1:ForEachPlayer( MoveToMarines ) // 
	Gamerules.team2:ForEachPlayer( MoveToMarines ) // 
	//Shine:NotifyColour( nil, 255, 160, 0, "FriendlyFire Enabled for the duration of the mapvote", true,
	Shared.ConsoleCommand("sh_friendlyfire 1")
end
		...
		if TimeLeft <= self.Config.ForceChange then
			if not self:VoteStarted() and not self.VoteOnEnd then
				Shine:NotifyColour( nil, 255, 160, 0, "The server will now cycle to %s.", true,
					self:GetNextMap() )
               Shared.ConsoleCommand("sh_friendlyfire 0")
...
function Plugin:JoinTeam( Gamerules, Player, NewTeam, Force, ShineForce )
	...
	if NewTeam == 1 then return end // marine
