...
function PathingMixin:CheckTarget(endPoint)
   ...
   local trace = Shared.TraceRay(self:GetOrigin(), endPoint, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButMixin("Moveable"))           
   local tracey = Shared.TraceRay(self:GetOrigin(), endPoint, CollisionRep.Move, PhysicsMask.AllButPCs, EntityFilterAllButIsa("FuncDoor"))                                                                                                                                                                                                                                                                 
      if trace.entity ~= nil  and ( trace.entity:isa("FrontDoor") or
      trace.entity:isa("LogicBreakable")  or
       trace.entity:isa("SiegeDoor") ) or 
           tracey.entity ~= nil and 
      ...
      /*
   if self:isa("TunnelEntrance") and endpoint.GetLocationName and
   string.find(endpoint:GetLocationName(), "siege") or string.find(endpoint:GetLocationName(), "Siege") then
   return false 
    end
    */
    
 //  if ( Shared.GetMapName() == "ns2_biodome_siege" or Shared.GetMapName() == "ns2_tram_siege" ) and (trace.entity ~= nil and trace.entity:isa("FuncMoveable")) then
 //   return false
 // end
...
// Create an absolute path using the points, ignoring the navigation mesh
// Used when you need to move things out from the inside of a robo-factory, for example
function PathingMixin:CreateAbsolutePath(pathPoints)
    local points = PointArray()
    local entity = self.builtEntity
    local lastPoint = nil
    for i,point in ipairs(pathPoints) do
        Pathing.InsertPoint( points, i, point )
        lastPoint = point
    end
    // the pathing system uses targetPoint as the target to move to 
    // and a PathCursor to move to that point. If the targetPoint differs
    // from the cursors endpoint, the unit will repath, so we need to
    // set both cursor and targetPoint to set the path correctly
    self.cursor = PathCursor():Init(points)
    self.targetPoint = lastPoint
end
