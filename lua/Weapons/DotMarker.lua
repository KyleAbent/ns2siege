...
local function ConstructTargetEntry(origin, hitEntity, damage, radius, ignoreLos, customImpactPoint, fallOffFunc)
    ...
    local entry = {}
        local tracex = Shared.TraceRay(origin, hitEntity:GetOrigin(), CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FuncMoveable"))
  //  local tracey = Shared.TraceRay(origin, hitEntity:GetOrigin(), CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FrontDoor"))  
   // local tracez = Shared.TraceRay(origin, hitEntity:GetOrigin(), PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FuncDoor")) 
   if not hitEntity or not hitEntity:GetIsAlive() or //or GetIsVortexed(hitEntity) or (tracey.entity ~= nil and tracey.entity:isa("FrontDoor")) or (tracex.entity ~= nil and tracex.entity:isa("FuncMoveable")) then
      (tracex.entity ~= nil and tracex.entity:isa("FuncMoveable")) then //or
        return nil
    end    
     if not hitEntity:isa("FuncDoor") then
         local tracez = Shared.TraceRay(origin, hitEntity:GetOrigin(), PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FuncDoor")) 
       if tracez.entity ~= nil and ( tracez.entity:isa("FuncDoor") and ( tracez.entity:GetState() == FuncDoor.kState.Welded or tracez.entity:GetState() == FuncDoor.kState.Locked ) ) then
       return nil 
       end
       end
    local worldImpactPoint = nil
