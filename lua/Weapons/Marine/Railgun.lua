...
local function ExecuteShot(self, startPoint, endPoint, player)
    ...
    //    local tracey = Shared.TraceRay(startPoint, endPoint, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FrontDoor")) 
        local tracet = Shared.TraceRay(startPoint, endPoint, CollisionRep.Move, PhysicsMask.LogicBreakable, EntityFilterAllButIsa("LogicBreakable")) 
        local tracex = Shared.TraceRay(startPoint, endPoint, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FuncMoveable")) 
       // local tracez = Shared.TraceRay(startPoint, startPoint + remainingRange * fireDirection, BigStructuresGroup, EntityFilterAllButIsa("FuncDoor")) 
         
    local extents = GetDirectedExtentsForDiameter(direction, kBulletSize)
    
   // if trace.fraction < 1 and not (tracey.entity ~= nil and tracey.entity:isa("FrontDoor")) and 
     if not (tracet.entity ~= nil and tracet.entity:isa("LogicBreakable")) and not (tracex.entity ~= nil and tracex.entity:isa("FuncMoveable")) then //and not (tracez.entity ~= nil and tracez.entity:isa("FuncDoor") and tracez.entity:GetState() == Door.kState.Locked) then
