...
function Shockwave:UpdateShockwave(deltaTime)
    ...
    local tracet = Shared.TraceRay(self:GetOrigin(), newPosition, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButMixin("FuncMoveable")) 
       local tracey = Shared.TraceRay(self:GetOrigin(), newPosition, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FuncDoor"))  
     if (newPosition - self.endPoint):GetLength() < 0.1 or 
      tracet.entity ~= nil and ( tracet.entity:isa("LogicBreakable") or tracet.entity:isa("FuncMoveable")) or 
      tracey.entity ~= nil and ( tracey.entity:isa("FuncDoor") and not ( tracey.entity:GetCanDamageGoThrough() ) ) then
       //   GetWallBetween(self:GetOrigin(), self.endPoint, tracey.entity) then
        DestroyShockwave(self)
     else
...
function Shockwave:Detonate()
    ...
    local endpoint = groundTrace.endPoint
    ...
    if groundTrace.fraction < 1 then
        ...
        for _, enemy in ipairs(enemies) do
            ...
            if enemy:GetIsAlive() and not table.contains(self.damagedEntIds, enemyId) and math.abs(enemy:GetOrigin().y - groundTrace.endPoint.y) < 0.8 and not GetWallBetween(origin, endpoint, enemy) then
