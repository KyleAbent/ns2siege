...
function GUIAnimatedScript:Initialize(noItemsUpdateInterval)

    self.guiItems = {}
    self.guiAnimatedItems = {}
    self.scale = Client.GetScreenHeight() / kBaseScreenHeight
    
    // set updateInterval BEFORE calling GUIAnimatedScript.Initialize() when scripts
    // need to run at a different update interval even when they do not have animated items
    self.noItemsUpdateInterval = noItemsUpdateInterval or kUpdateIntervalLow
...
function GUIAnimatedScript:Update(deltaTime)
    ...
    if numAnimatingtems == 0 then
        self.updateInterval = self.noItemsUpdateInterval
    else
