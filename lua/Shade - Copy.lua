...
function Shade:GetTechButtons(techId)
    ...
    local techButtons = { kTechId.ShadeInk, kTechId.Move, kTechId.ShadeCloak, kTechId.None, 
     ...
     if not self.inktrigger then
         techButtons[5] =    
         kTechId.InkTriggerOn
     else
         techButtons[1] =
         kTechId.InkTriggerOff
         techButtons[5] = 
         kTechId.None
     end
  ...
  function Shade:GetUnitNameOverride(viewer)
    ...
    if self.inktrigger then //and not self:GetCanAutomaticTriggerInkAgain() then
...
function Shade:OnScan()
    self:PerformAutomaticInkTrigger()
end
local function GetCommander(teamNum)
    local commanders = GetEntitiesForTeam("Commander", teamNum)
    return commanders[1]
end
function Shade:PerformAutomaticInkTrigger()
       if not self:GetCanAutomaticTriggerInkAgain() then return end
       ...
       if commander then
       commander:SetTechCooldown(kTechId.ShadeInk, cooldown, Shared.GetTime())
       end
       self:TellOtherShadesToAbide()
end
function Shade:TellOtherShadesToAbide()
       local shades = GetEntitiesForTeam("Shade", 2)
       shades.lastinktrigger = Shared.GetTime()
end
...
function Shade:GetCanAutomaticTriggerInkAgain()
        return self:GetHasShadeHive() and self.inktrigger and Shared.GetTime() > self.lastinktrigger + self:GetCoolDown()
end
...
function Shade:FlipInkTrigger()

    self.inktrigger = not self.inktrigger
    return true
...
function Shade:PerformActivation(techId, position, normal, commander)
    ...
    elseif techId == kTechId.InkTriggerOn then
           success = self:FlipInkTrigger()
    elseif techId == kTechId.InkTriggerOff then
           success = self:FlipInkTrigger()
    end
