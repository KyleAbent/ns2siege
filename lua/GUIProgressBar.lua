...
GUIProgressBar.kFontName = Fonts.kAgencyFB_Medium
...
local function UpdateItemsGUIScale(self)
    ...
    kSmokeyBackgroundSize = GUIScale(Vector(400, 200, 0)) * kScale
    kSmokeyBackgroundPos = -kSmokeyBackgroundSize * .5 + GUIScale(Vector(0, -100, 0)) * kScale
    
    kBorderMaskCircleRadius = GUIScale(140)
    
    kShineSize = GUIScale(Vector(400, 150, 0)) 
    kShinePos = Vector(-kShineSize.x * .5, -GUIScale(105), 0)
end

function GUIProgressBar:OnResolutionChanged(oldX, oldY, newX, newY)
    self:Uninitialize()
    self:Initialize()
end
...
function GUIProgressBar:Initialize()
    ...
    GUIMakeFontScale(self.objectiveTextShadow)
    ...
    GUIMakeFontScale(self.objectiveText)
