...
local function createTab(index, text, size)
    ...
    tabBar:SetSize(Vector(size/3, GUIScale(50), 0))
    tabBar:SetPosition(Vector(index * size/3, -GUIScale(50), 0))
...
function GUIInsight_Graphs:OnResolutionChanged(oldX, oldY, newX, newY)
    self:Uninitialize()
    graphPadding = GUIScale(50)
...
function GUIInsight_Graphs:Initialize()
    ...
    local graphSize = GUIScale(Vector(800, 400, 0))
    ...
    tabSelected = 1
    tabBars[1] = createTab(0, Locale.ResolveString("RESOURCE_TOWERS"), graphSize.x)
    tabBars[1]:SetColor(selectedColor)
    tabBars[2] = createTab(1, Locale.ResolveString("TEAM_RESOURCES"), graphSize.x)
    tabBars[3] = createTab(2, Locale.ResolveString("TEAM_KILLS"), graphSize.x)
...
function GUIInsight_Graphs:GetIsVisible()
    return isVisible
end
...
function GUIInsight_Graphs:Update(deltaTime)
    ...
    PROFILE("GUIInsight_Graphs:Update")
