...
local function UpdateDesiredCloakFraction(self, deltaTime)
    ...
    if Server and not ( self:isa("Fade") and self:GetEligableForProlongedInvisibility() ) then
    ...
    if not ( self:isa("Fade") and self:GetEligableForProlongedInvisibility() ) then 
        ...
        self.cloakFraction = newCloak
           if callOnCloak then
            ...
            if GetHasCamouflageUpgrade(self) then
                  ...
                  self.cloakFraction = kPlayerMaxCloak
                  elseif GetVeilLevel(2) == 2 then
                  self.cloakFraction = kPlayerMaxCloak /2
                  elseif GetVeilLevel(2) == 1 then
                  self.cloakFraction = kPlayerMaxCloak / 3
                  end
            end
...
elseif Client then
    ...
    function CloakableMixin:_UpdatePlayerModelRender(model)
        ...
        local useMaterial = (self.cloakingDesired or self:GetCloakFraction() ~= 0  or ( self:isa("Fade") and self:GetEligableForProlongedInvisibility() ) ) and not hideFromEnemy
...
function CloakableMixin:SecondaryAttack()
    ...
    if weapon and weapon:GetHasSecondary(self) and not ( self:isa("Fade") and self:GetEligableForProlongedInvisibility() ) then
