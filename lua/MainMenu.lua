...
function MainMenu_GetMusicList()
    local menuMusic = {"Default", "Eclipse Remix", "Exo (Original Music)", "Beta", "NS1", "Frontiersmen", "Random" }
    return menuMusic
end
...
function MainMenu_OnOpenMenu()
   if not gMainMenu.newsScript then
    gMainMenu.newsScript = GetGUIManager():CreateGUIScript("menu/GUIMainMenuNews")
    else
    gMainMenu.newsScript:SetIsVisible(gMainMenu.Links[1]:GetIsVisible())
    end
    StartSoundEffect(kLoopingMenuSound)    
end

function MainMenu_OnCloseMenu()
         if gMainMenu and gMainMenu.newsScript then
		 GetGUIManager():DestroyGUIScript(gMainMenu.newsScript)
		 gMainMenu.newsScript:Uninitialize()
		 gMainMenu.newsScript = nil
	     end
