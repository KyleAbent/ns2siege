...
function GUIInsight_TechPoints:OnResolutionChanged(oldX, oldY, newX, newY)
    ...
    hBarHeight = GUIScale(15)
    kTechPointSize = GUIScale(Vector(225, 40, 0))
    kIconSize = GUIScale(Vector(40, 40, 0))
    kTeamNameFontScale = GUIScale(Vector(1, 0.8, 0))
    kTeamInfoFontScale = GUIScale(Vector(0.9, 0.7, 0))
...
function GUIInsight_TechPoints:Update(deltaTime)
    ...
    PROFILE("GUIInsight_TechPoints:Update")
...
function GUIInsight_TechPoints:UpdateTechPoint(techPoint, techPointRecord, currentPointIndex)
    ...
    if teamChanged then
        ...
        if team > 0 then        
            ...
            if not built then
                ...
                local info = {Text = text, Scale = Vector(1,1,1), Color = storedValues.TeamColor, ShadowColor = Color(0,0,0,0.5)}
            ...
            if storedValues.Team > 0 then
                ...
                local alertinfo = {Text = text, Scale = Vector(1,1,1), Color = storedValues.TeamColor, ShadowColor = Color(0,0.5,0.5,0.5)}
                local position = techPoint.Background:GetScreenPosition(Client.GetScreenWidth(), Client.GetScreenHeight())
            ...
            if alive then
                ...
                if not built then
                    ...
                    if team == kTeam1Index then
                        ...
                        builtStr = Locale.ResolveString("TECHPOINT_GROWN")
                    end
                    infoText = string.format("%s (%d%% %s)", infoText, math.ceil(builtFraction*100), builtStr)
                    
                elseif storedValues.Built < builtFraction then -- just finished building
                
                    local text = string.format("%s %s", location, Locale.ResolveString("TECHPOINT_FINISHED"))
                    local icon = {Texture = kIconTexture, TextureCoordinates = GetTextureCoords(team, techId), Color = Color(1,1,1,0.5), Size = kIconSize}
                    local alertinfo = {Text = text, Scale = Vector(1,1,1), Color = storedValues.TeamColor, ShadowColor = Color(0,0.5,0.5,0.5)}
                    local position = techPoint.Background:GetScreenPosition(Client.GetScreenWidth(), Client.GetScreenHeight())
                ...
                infoText = Locale.ResolveString("TECHPOINT_DESTROYED")
                info:SetColor(kDeadColor)

            end

        else
            infoText = Locale.ResolveString("TECHPOINT_UNCLAIMED")
            info:SetColor(kInfoColor)
        end
    ...
    if team == kTeam1Index then
        if powerFraction ~= storedValues.Power then
            ...
            local powerText = string.format( Locale.ResolveString("INSIGHT_MARINE_POWER"), math.ceil(powerFraction*100))
    ...
    elseif team == kTeam2Index then
        ...
        if eggcount ~= storedValues.eggCount then
            techPoint.Info2:SetText(string.format( Locale.ResolveString("INSIGHT_ALIENS_EGGS"), eggcount))
...
function GUIInsight_TechPoints:CreateBackground()
    ...
    infoItem:SetAnchor(GUIItem.Left, GUIItem.Center)
