...
function GUIInsight_AlertQueue:Initialize()
    ...
    kIconSize = GUIScale(Vector(96, 96, 0))
    kIconPadding = GUIScale(Vector(0, 8, 0))
    kInfoScale = GUIScale(Vector(1, 1, 1))
    kAlertOffset = Vector(0, -(kIconSize.y + GUIScale(32)), 0)
    ...
    alertContainer:SetColor(Color(1, 1, 1, 0))
...
function GUIInsight_AlertQueue:SetIsVisible(bool)
    ...
    alertContainer:SetColor(Color(1, 1, 1, isVisible and 1 or 0))
...
function GUIInsight_AlertQueue:Update(deltaTime)
    ...
    PROFILE("GUIInsight_AlertQueue:Update")
...
function GUIInsight_AlertQueue:CreateAlert(position, icon, info, team)
    ...
    if info then
        ...
        GUIMakeFontScale(infoShadowItem)
